...
public:
	inline void initial(ModelParams& params, HyperParams& hyparams) {
		...
		for (int idx = 0; idx < length; idx++) {
			...
			char_left_represents[idx].init(hyparams.char_represent_dim, -1);
			char_right_represents[idx].init(hyparams.char_represent_dim, -1);
		...
		bichar_nullkey.init(hyparams.bichar_dim, hyparams.dropProb);
	}
